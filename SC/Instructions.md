...
The type of relationship between the Employee and Territory tables is one-to-many. For example, each EmployeeID can align or match up with more than one TerritoryID. Specifically, we observed in Part 3 that EmployeeID 7 had 10 distinct TerritoryIDs.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

Document oriented databases or document stores are great for storing large amounts of documents,generally unstructured or semi-structured content. This is a non-relational approach to storing data and is appropriate under certain circumstances. For example, in situations where you want to prototype quickly/develop something as fast as possible and have a higher likelihood of needing to rewrite, a document store could be an ideal choice. A specifc situtation where document stores are not appropriate are banks, as they should be well-defined in their data(have an upfront schema).

- What is "NewSQL", and what is it trying to achieve?

NewSQL attempts to achieve the best of both worlds between relational and non-relational databases. Specifically, it seeks to provide the scalability of NoSQL and a set of properties associated with traditional SQL known as ACID (Atomicity, Consistentcy, Isolation, Durability. With innovation constantly in motion, NewSQL will likely continue to update and reestablish what we are coining as the best of both worlds.
